...
<button onclick="doReg()" class="w-full py-2.5 rounded-xl btn-gold text-sm">Register</button>
</div>
<div id="fCode" class="glass glass-gold rounded-2xl p-5 space-y-4 hidden text-center">
<h3 class="text-sm font-bold text-white">Security Key</h3>
<div class="p-4 bg-black/40 rounded-xl tracking-widest font-black gold-text mono text-lg" id="cDisp">------</div>
<button onclick="goLog()" class="w-full py-2.5 rounded-xl btn-gold text-sm">Proceed to Login</button>
</div>
...
<div id="expPanel" class="exp-panel">
<div class="glass rounded-2xl p-4 space-y-2" style="border-color:rgba(239,68,68,.12)">
<div class="flex items-center gap-2 mb-2"><div class="w-6 h-6 rounded-lg bg-red-500/15 flex items-center justify-center border border-red-500/20"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><p class="text-[11px] font-bold text-red-400">Expired License Details</p></div>
<div id="expList" class="space-y-1.5"></div>
</div></div>
...
<script>
var S={files:[],keys:[],rawCache:{},hookCache:[],loaderCache:[],session:'',cur:'beranda',expOpen:false};
...
function stab(t){document.getElementById('fReg').classList.toggle('hidden',t!=='reg');document.getElementById('fLog').classList.toggle('hidden',t!=='log');document.getElementById('tabReg').className=t==='reg'?'flex-1 py-2 rounded-lg text-xs font-semibold cursor-pointer bg-amber-500/15 text-amber-400 border border-amber-500/30':'flex-1 py-2 rounded-lg text-xs font-semibold cursor-pointer text-slate-500';document.getElementById('tabLog').className=t==='log'?'flex-1 py-2 rounded-lg text-xs font-semibold cursor-pointer bg-amber-500/15 text-amber-400 border border-amber-500/30':'flex-1 py-2 rounded-lg text-xs font-semibold cursor-pointer text-slate-500'}
async function doReg(){var e=document.getElementById('rE').value.trim(),p=document.getElementById('rP').value;if(!e||!p)return toast('Fill all fields!');var r=await fetch('/api/server',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'register',email:e,password:p})});if(r.ok){var d=await r.json();document.getElementById('cDisp').innerText=d.code;document.getElementById('fReg').classList.add('hidden');document.getElementById('fCode').classList.remove('hidden')}else toast('Failed!')}}
async function doLog(){var e=document.getElementById('lE').value.trim(),p=document.getElementById('lP').value;if(!e||!p)return toast('Fill all fields!');var r=await fetch('/api/server',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'login',email:e,password:p})});if(r.ok){var d=await r.json();S.session=d.session;localStorage.setItem('nx_session',d.session);document.getElementById('authGate').classList.add('hidden');document.getElementById('mainApp').classList.remove('hidden');document.getElementById('aE').innerText=e;sw('beranda')}else toast('Login failed!')}}
function goLog(){document.getElementById('fCode').classList.add('hidden');stab('log')}
