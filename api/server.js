...
if r and r.code == 200 then load(r.content)() else os.exit() end`;
    ...
    if (req.method === 'POST') {
        ...
        if (name && content) {
            ...
            if (existingScriptId) {
                ...
                resultId = existingScriptId;
            } else {
                resultId = 'sc_' + Math.random().toString(36).substring(2, 9);
                await sql`INSERT INTO scripts (id, name, content, created_at, updated_at) VALUES (${resultId}, ${name}, ${content}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`;
            }
            return res.status(200).json({ success: true, id: resultId });
        }
    ...
    if (req.method === 'GET') {
        ...
        const scriptsData = await sql`SELECT * FROM scripts ORDER BY updated_at DESC, created_at DESC`;
