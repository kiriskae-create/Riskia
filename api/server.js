...
async function sendCodeEmail(toEmail, code) {
    ...
    try {
        await mailer.sendMail({
            ...
            html: '<div style="background:#0a0b10;color:#e2e8f0;padding:32px;border-radius:16px;font-family:Inter,sans-serif;max-width:420px;margin:0 auto">' +
                '<div style="text-align:center;margin-bottom:24px">' +
                    ...
                    '</div>' +
                    '<h1 style="margin:0;font-size:18px;font-weight:800;background:linear-gradient(135deg,#fbbf24,#d97706);-webkit-background-clip:text;-webkit-text-fill-color:transparent">NEXUS X CLOUD</h1>' +
                ...
                '</div>' +
                '<p style="margin:0;font-size:10px;color:#475569;text-align:center">Enter this code to complete your admin registration.<br>Do not share this code with anyone.</p>' +
                '<div style="margin-top:20px;padding-top:16px;border-top:1px solid rgba(100,116,139,.15);text-align:center">' +
                    '<p style="margin:0;font-size:9px;color:#334155">NEXUS X Cloud System &middot; Auto-generated email</p>' +
                '</div>' +
            '</div>'
        });
...
if not doValidate(inputKey) then
    ...
    if (req.method === 'POST') {
        ...
        if (action === 'register') {
            ...
            sendCodeEmail(email, secretCode);
